    ...
    return (
        <div className='bg-black/50  fixed  top-0 left-0 z-50 items-center justify-center h-screen w-full text-black flex flex-col'>
            <div className='bg-white h-auto w-full md:w-1/4 rounded pb-3'>
                <span className='p-2 px-3 flex items-center justify-between border-b border-[#A5A5A5] font-medium text-[#363636]'>{item.name} ({item.size})
                    <button
                ...
                <div className='grid gap-2 px-3 py-2 pb-4 border-b border-[#A5A5A5]'>
                    ...
                    <div className='flex items-center flex-wrap gap-2 px-2 py-2 border border-[#A5A5A5] rounded min-h-[40px]'>
                        {notes.map((note, index) => (
                            <div
                                key={index}
                                className='flex items-center bg-[#56A65C] text-white px-5 py-1 rounded gap-1'
                            >
                                <button
